...
console.log("Team List", team_members_list);

const $teamWrapper = $('.js-team-wrapper')[0];
const $teamWrapperColumnOne = $('.js-team-wrapper-column-one')[0];
const $teamWrapperColumnTwo = $('.js-team-wrapper-column-two')[0];
const ssm = __webpack_require__(7);

if (team_members_list && team_members_list.length && team_members_list.length > 0) {
    const expandedTeamMemberList = team_members_list.map(teamMeber => expandTeamMemberData(teamMeber));
    const sortedTeamMemberList = expandedTeamMemberList.sort(compareWeight);

    ssm.addState({
        id: 'minWidthLarge',
        query: {
            minWidth: '(min-width: 641px)'
        },
        onLeave: renderSingleColumn(sortedTeamMemberList),
        onEnter: renderTwoColumn(sortedTeamMemberList)
    });
}
...
function renderTeamMember(element, teamMemberData) {
    ...
    const teamMemberTemplate = `
        <div class="team__individual-wrapper">
            <div class="team__individual-image" style="background-image: url(${teamMemberData.imageUrl})"/></div>

            <div class="team__individual">
                <h2 class="team__individual-name padding-top-30">${teamMemberData.name} ${teamMemberData.surname}</h2>
                <h3 class="team__individual-role margin-top-0">${teamMemberData.position}</h3>
...
function renderSingleColumn(sortedTeamMemberList) {
    console.log('onLeave renderSingleColumn');
}

function renderTwoColumn(sortedTeamMemberList) {
...
/***/ })
/******/ ]);
//# sourceMappingURL=main-8a39952f6d1d67faca3c.js.map