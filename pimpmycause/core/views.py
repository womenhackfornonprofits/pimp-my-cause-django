...
def homepage(request):
    ...
    featured_marketer_list = (
        PimpUser.objects.filter(featured=True)
    ...
    news_post_list = NewsPost.objects.order_by("-created_at")[:3]
...
def team_member_list(request):
    team_member_list_json = TeamMember.objects.values_list(
        "name", "surname", "bio", "image", "position", "visual_priority", "group"
    ).order_by("visual_priority")

    team_member_list = TeamMember.objects.order_by("visual_priority")
